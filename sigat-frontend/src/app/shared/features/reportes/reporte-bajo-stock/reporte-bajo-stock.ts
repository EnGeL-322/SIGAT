...
export class ReporteBajoStockComponent implements OnInit {
  ...
  exportarPdf(): void {
    const rows = this.filtrados.map(item => `<tr><td>${item.marca}</td><td>${item.modelo}</td><td>${item.codigo}</td><td>${item.stockActual}</td><td>${item.stockMinimo}</td><td>${this.estadoProducto(item.stockActual)}</td></tr>`).join('');
    const win = window.open('', '_blank');
    win?.document.write(`<html><head><title>Reporte de bajo stock</title><style>body{font-family:Arial;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:8px}th{background:#7d82ef;color:white}</style></head><body><h1>Reporte de bajo stock</h1><p>Fecha: ${this.fecha}</p><table><thead><tr><th>Marca</th><th>Modelo</th><th>Codigo</th><th>Stock actual</th><th>Stock minimo</th><th>Estado</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
    win?.document.close();
    win?.print();
  }
}
